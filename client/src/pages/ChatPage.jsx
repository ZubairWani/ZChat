...
//       const names = conversationId.split('-');
//       const partner = names.find(name => name !== user.name.toLowerCase());
//       setChatPartnerName(partner.charAt(0).toUpperCase() + partner.slice(1));
//     } else {
//       setChatPartnerName(user.otherName);
//     }
...
//                     {isConnected ? 'Secure Chat' : connectionStatus}
//                   </span>
//                 </div>
//               </div>
//             </div>
      ...
      <div className="relative z-10 flex flex-col h-screen">
        ...
        `}>
          <div className="flex items-center justify-between max-w-4xl mx-auto">
            <button 
              onClick={handleGoBack} 
              className="p-2 sm:p-3 rounded-full backdrop-blur-sm bg-white/5 border border-white/10 hover:bg-white/10 transition-all duration-300 transform hover:scale-105 active:scale-95"
              aria-label="Go back"
            >
              <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5 sm:h-6 sm:w-6 text-white" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 19l-7-7 7-7" />
        ...
        <main className={`flex-1 overflow-hidden transition-all duration-1000 ease-out ${isVisible ? 'translate-y-0 opacity-100' : 'translate-y-8 opacity-0'}`}>
          <div className="h-full overflow-y-auto p-4 sm:p-6 max-w-4xl mx-auto">
            <div className="space-y-4 sm:space-y-6">
              {messages.length > 0 ? (
                messages.map((msg, index) => (
                  <div key={msg._id || index} className="transition-all duration-500 ease-out">
                    <MessageBubble message={msg} isOwnMessage={msg.senderName === user.name} />
                  </div>
                ))
              ) : (
                <div className={`flex flex-col items-center justify-center h-full text-center backdrop-blur-sm bg-white/5 border border-white/10 rounded-3xl p-8 sm:p-12 transition-all duration-1000 ease-out ${isVisible ? 'translate-y-0 opacity-100' : 'translate-y-8 opacity-0'}`}>
                  <div className="inline-flex items-center justify-center w-16 h-16 sm:w-20 sm:h-20 mb-6 bg-gradient-to-br from-blue-600 to-cyan-600 rounded-2xl shadow-lg">
                    <svg className="w-8 h-8 sm:w-10 sm:h-10 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M8 12h.01M12 12h.01M16 12h.01M21 12c0 4.418-4.03 8-9 8a9.863 9.863 0 01-4.255-.949L3 20l1.395-3.72C3.512 15.042 3 13.574 3 12c0-4.418 4.03-8 9-8s9 3.582 9 8z" />
                    </svg>
        ...
        <div className={`backdrop-blur-xl bg-white/5 border-t border-white/10 p-4 sm:p-6 transition-all duration-1000 ease-out ${isVisible ? 'translate-y-0 opacity-100' : 'translate-y-8 opacity-0'}`}>
          <div className="max-w-4xl mx-auto">
            <ChatInput onSendMessage={handleSendMessage} />
          </div>
        </div>
